...
G4OpticalSurface * PMTLogicalVolume::our_Mirror_opsurf = nullptr;

////////////////////////////////////////////////////////////////
// PMTLogicalVolume
//
PMTLogicalVolume::PMTLogicalVolume(
  ...
  : G4LogicalVolume(new G4Tubs(plabel + "_envelope_solid", 0.0, r_bound, hh_bound, 0., twopi),
                    ExteriorMat, plabel)
{
  if (our_Mirror_opsurf == nullptr) {
    ...
    G4MaterialPropertiesTable * propMirror = nullptr;
...
//
PMT17inchLogicalVolume::PMT17inchLogicalVolume(
    const G4String & plabel,                // label -- subvolume names are derived from this
    G4Material * ExteriorMat,               // material which fills the bounding cylinder
  ...
  : PMTLogicalVolume(plabel, 260. * millimeter,
                     ...
                     ExteriorMat)
{
  ...
  if (MaskMat != nullptr) {
    ...
    G4double r_mask_outer =
        static_cast<G4Tubs *>(this->GetSolid())->GetOuterRadius(); // bounding cylinder size
    ...
    PMTTorusStack * facemask_inner_solid = new PMTTorusStack(plabel + "_facemask_inner_solid");
    facemask_inner_solid->SetAllParameters(2, z_edge, rho_edge, z_o);

    rho_edge[0] += 2.0 * mm;
    rho_edge[1] += 2.0 * mm;
    rho_edge[2] += 2.0 * mm;
    PMTTorusStack * facemask_solid = new PMTTorusStack(plabel + "_facemask_solid");
...
//
PMT20inchLogicalVolume::PMT20inchLogicalVolume(
    const G4String & plabel,                // label -- subvolume names are derived from this
    G4Material * ExteriorMat,               // material which fills the bounding cylinder
    G4Material * GlassMat,                  // glass material
    G4OpticalSurface * Photocathode_opsurf, // photocathode
...
//
PMT10inchLogicalVolume::PMT10inchLogicalVolume(
    const G4String & plabel,                // label -- subvolume names are derived from this
    G4Material * ExteriorMat,               // material which fills the bounding cylinder
    G4Material * GlassMat,                  // glass material
    G4OpticalSurface * Photocathode_opsurf, // photocathode
  ...
  : PMTLogicalVolume(plabel, 126.5 * millimeter, 165. * millimeter, ExteriorMat)
{
...
////////////////////////////////////////////////////////////////
// PMT8inchLogicalVolume
//
PMT8inchLogicalVolume::PMT8inchLogicalVolume(
    const G4String & plabel,                // label -- subvolume names are derived from this
    G4Material * ExteriorMat,               // material which fills the bounding cylinder
    G4Material * GlassMat,                  // glass material
    G4OpticalSurface * Photocathode_opsurf, // photocathode
  ...
  : PMTLogicalVolume(plabel, 110. * millimeter, 150. * millimeter, ExteriorMat)
{
...
////////////////////////////////////////////////////////////////
// PMT5inchLogicalVolume
//
PMT5inchLogicalVolume::PMT5inchLogicalVolume(
    const G4String & plabel,                // label -- subvolume names are derived from this
    G4Material * ExteriorMat,               // material which fills the bounding cylinder
    G4Material * GlassMat,                  // glass material
    G4OpticalSurface * Photocathode_opsurf, // photocathode
  ...
  : PMTLogicalVolume(plabel, 64. * millimeter, 108. * millimeter, ExteriorMat)
{
...
//
void PMTLogicalVolume::ConstructPMT_UsingTorusStack(
...
)
{
  ...
  PMTTorusStack * body_solid = new PMTTorusStack(GetName() + "_body_solid");
  body_solid->SetAllParameters(n_edge, outer_z_edge, outer_rho_edge, outer_z_o);

  // inner volumes (to be filled with vacuum)
  PMTTorusStack * inner1_solid = new PMTTorusStack(GetName() + "_inner1_solid");
  PMTTorusStack * inner2_solid = new PMTTorusStack(GetName() + "_inner2_solid");

  // set shapes of inner volumes
  // also scan for lowest allowed point of dynode
  G4double z_lowest_dynode = z_dynode;
  {
    // Workspace for inner surface edge calculations.
    std::vector<G4double> dscratch(4 * (n_edge + 1));
    G4double * inner_z_edge = dscratch.data();
    G4double * inner_rho_edge = dscratch.data() + n_edge + 1;
    G4ThreeVector norm;
    ...
    if (iedge_equator < 0) {
      ...
      G4cerr << "PMTLogicalVolume::ConstructPMT_UsingTorusStack: "
    ...
    // PMTTorusStack keeps its own copy of edges; dscratch goes out of scope here.
  }
  ...
  z_equator = static_cast<G4Tubs *>(this->GetSolid())->GetZHalfLength() - outer_z_edge[0] -
  ...
  // FastSimulationModel
  // setup optical model
  G4Region * body_region = new G4Region(GetName() + "_PMTOpticalRegion");
  body_region->AddRootLogicalVolume(body_log);
  new PMTOpticalModel(GetName() + "_optical_model", body_region, body_log, OpPCSurface);
