...
#include "G4ios.hh"
#include "PMTTorusStack.hh"
#include "meshdefs.hh"

namespace
{
inline double square(double x) { return x * x; }
inline bool samesign(double x, double y) { return (x * y > 0.0); }
} // namespace

double PMTTorusStack::surfaceTolerance = 0.0;
double PMTTorusStack::angTolerance = 0.0;
double PMTTorusStack::radTolerance = 0.0;

// Constructor
PMTTorusStack::PMTTorusStack(const G4String & pName)
  : G4CSGSolid(pName)
{
...
void PMTTorusStack::SetAllParameters(G4int n_,                 // number of Z-segments
                                      ...
                                      PMTTorusStack * inner_)  // inner TorusStack to subtract
{
  int i;
  int n_increasing;
  ...
  if (n_increasing != n) {
    G4Exception(__FILE__, "Invalid Parameter", FatalException,
                "Error in PMTTorusStack::SetAllParameters - "
    ...
    if (b[i] > 0) {
      ...
      if (rho_edge[i] < a[i]) {
        if (fabs(a[i] - rho_edge[i]) > max_rho * 1e-3 + radTolerance)
          G4cerr << "Warning: PMTTorusStack making sizeable adjustment to "
                    "rho_edge["
                 << i << "]: old " << rho_edge[i] << ", new " << a[i] << " (1)" << G4endl;
      ...
      if (rho_edge[i + 1] < a[i]) {
        if (fabs(a[i] - rho_edge[i + 1]) > max_rho * 1e-3 + radTolerance)
          G4cerr << "Warning: PMTTorusStack making sizeable adjustment to "
                    "rho_edge["
                 << i + 1 << "]: old " << rho_edge[i + 1] << ", new " << a[i] << " (2)" << G4endl;
        // valid intercept is on other side of a[i]
      ...
      if (rho_edge[i] > a[i]) {
        if (fabs(a[i] - rho_edge[i]) > max_rho * 1e-3 + radTolerance)
          G4cerr << "Warning: PMTTorusStack making sizeable adjustment to "
                    "rho_edge["
                 << i << "]: old " << rho_edge[i] << ", new " << a[i] << " (3)" << G4endl;
        // valid intercept is on other side of a[i]
      ...
      if (rho_edge[i + 1] > a[i]) {
        if (fabs(a[i] - rho_edge[i + 1]) > max_rho * 1e-3 + radTolerance)
          G4cerr << "Warning: PMTTorusStack making sizeable adjustment to "
                    "rho_edge["
                 << i + 1 << "]: old " << rho_edge[i + 1] << ", new " << a[i] << " (4)" << G4endl;
        // valid intercept is on other side of a[i]
...
void PMTTorusStack::ComputeDimensions(G4VPVParameterisation *, const G4int,
                                       const G4VPhysicalVolume *)
{
  G4cerr << "Warning: ComputeDimensions is not defined for PMTTorusStack. "
...
class PMTTorusStack_TorusFunc : public PMTTorusStack::RootFinder {
public:
  G4double rr, ru, uu, z, w;
  G4double a, b;
  void f_and_Df(G4double s, G4double & f, G4double & Df) override
  {
...
G4int PMTTorusStack::FindFirstTorusRoot(
    ...
    G4double & sout)         // distance to root, if found
{
  ...
  if (b == 0.0) { // special case: cylinder
    if (tfunc.uu == 0.0 && tfunc.ru == 0.0) return 0;
    else {
      ...
      if (A == 0.0) sout = -C / B; // tangent -- only one root
      else {
        C = B * B - 4 * A * C;
        if (C < 0.0) // no intersection
          ...
          B /= 2 * A;
          C /= 2 * A;
          if (fEntering) sout = -B - C; // want first root if looking for entrance point
          else sout = -B + C;           // want second root if looking for exit point
        }
      }
  ...
  if (a == 0.0) { // special case: sphere
    ...
    if (C < 0.0) // no intersection
      ...
      B /= 2.;
      C /= 2.;
      if (fEntering) sout = -B - C; // want first root if looking for entrance
      else sout = -B + C;           // want second root if looking for exit
    }
    return (sout >= smin && sout <= smax) ? 1 : 0;
  }
...
int PMTTorusStack::RootFinder::FindRoot(G4double smin, G4double smax, G4double tol,
...
retry: // will retry at most once
  ...
  if ((samesign(f1, f2)) || (oldds > tol)) {
    ...
    G4double ds = (smax - smin) / 640.0; // we need plenty of steps in order to ensure the boundary
                                         // is found otherwise we will obtain a strangely shaped PMT
...
G4int PMTTorusStack::FindInOrderedList(double z_lu, const double * z, int n)
{
  int a, b, c;
...
G4int PMTTorusStack::FindNearestSegment(G4double pr, G4double pz) const
{
  G4double distmin = kInfinity;
  G4int imin = -1;
  G4int i0 = FindInOrderedList(pz, z_edge.data(), n + 1);
  G4int i;

  if (i0 < 0) {
    if (pz >= z_edge[n]) i0 = n - 1;
    else i0 = 0;
  }
...
G4bool PMTTorusStack::CalculateExtent(const EAxis pAxis, const G4VoxelLimits & pVoxelLimit,
...
EInside PMTTorusStack::Inside(const G4ThreeVector & p) const
{
  ...
  if (inner) {
    ...
    if (in2 == kInside) return in = kOutside;
    else if (in2 == kOutside) return in;
    else return (in == kSurface) ? kOutside : kSurface;
  }

  return in;
}

EInside PMTTorusStack::Inside1(const G4ThreeVector & p) const
{
  EInside in = kInside;
  G4int i;
  ...
  else i = FindInOrderedList(p.z(), z_edge.data(), n + 1); // find z in table

  // 2-d distance from axis
  G4double rp = std::hypot(p.x(), p.y());
  ...
  if (drtor > 0.5 * myRadTolerance) return (kOutside);
  else if (drtor > -0.5 * myRadTolerance) return (kSurface);
...
G4ThreeVector PMTTorusStack::SurfaceNormal(const G4ThreeVector & p) const
{
  ...
  if (i == -1) { // bottom surface
    ...
    if (b[i] < 0.0) // handle concave surface
      norm = -norm;
    if ((b[i] < 0.0) != (dr < 0.0)) {
      G4cerr << "Warning from PMTTorusStack::SurfaceNormal: position "
...
G4double PMTTorusStack::DistanceToIn(const G4ThreeVector & p, const G4ThreeVector & v) const
{
  ...
  if (inner) {
    ...
    if (inner->Inside1(p) != kOutside) {
      ...
      if (Inside1(p + dist_to_in * v) == kInside) return dist_to_in;
      else return kInfinity; // no intersection
    }
  }
  // note: if we are here, then inner==nullptr or inner->Inside(p)==kOutside
  ...
  if (p.z() > z_edge[n] - surfaceTolerance) { // above top of torus stack
    ...
    iedge = FindInOrderedList(p.z(), z_edge.data(), n + 1); // find z in table
    idir = (v.z() >= 0.0) ? +1 : -1;
  }
  ...
  if (rmin2 > square(max_rho + myRadTolerance)) return kInfinity;
  ...
  if (p.z() > z_edge[n] - surfaceTolerance && v.z() < 0.0 && square(rho_edge[n]) >= rmin2) {
    ...
    if (square(pi.x()) + square(pi.y()) <= square(rho_edge[n])) {
      if (inner == nullptr || inner->Inside1(pi) == kOutside) {
        dist_to_in = distZ;
...
G4double PMTTorusStack::DistanceToIn(const G4ThreeVector & p) const
{
  G4double safe;
  G4double pr, pz;
  ...
  if (safe == 0.0 && inner != nullptr && inner->Inside1(p) == kInside) {
...
G4double PMTTorusStack::DistanceToOut(const G4ThreeVector & p, const G4ThreeVector & v,
                                       ...
                                       G4ThreeVector * norm) const
{
  ...
  if (calcNorm && validNorm != nullptr) { *validNorm = false; }
  isurface = -2;

  // find where we are in z
  if (p.z() > z_edge[n] + 0.5 * myRadTolerance) { return 0.0; }
  if (p.z() < z_edge[0] - 0.5 * myRadTolerance) { return 0.0; }
  ...
  // check to see if we're totally outside
  if (rp2 > square(max_rho + myRadTolerance)) { return 0.0; }
  ...
  if (calcNorm && norm) {
      ...
      if (pr == 0.0) { // point is on z-axis -- it is silly!
        ...
        *validNorm = true; /* (b[isurface] > 0.0); */
        if (dr != 0.0 && (b[isurface] < 0.0) != (dr < 0.0)) {
          G4cerr << "Warning from PMTTorusStack::DistanceToOut (surface "
    ...
    if ((*norm) * v <= 0.0) {
      G4cerr << "Warning from PMTTorusStack::DistanceToOut: I have calculated "
...
G4double PMTTorusStack::DistanceToOut(const G4ThreeVector & p) const
{
  G4double safe;
  G4double pr, pz;
...
//
G4ThreeVectorList * PMTTorusStack::CreateRotatedVertices(const G4AffineTransform & pTransform,
                                                          G4int & noPolygonVertices) const
{
  ...
  if (noCrossSections < kMinMeshSections) { noCrossSections = kMinMeshSections; }
  ...
  if (vertices) {
    ...
    G4Exception(__FILE__, "Out of Memory", FatalException,
                "PMTTorusStack::CreateRotatedVertices Out of memory - Cannot "
                "alloc vertices");
  }
  return vertices;
}

void PMTTorusStack::DescribeYourselfTo(G4VGraphicsScene & scene) const
{
...
G4VisExtent PMTTorusStack::GetExtent() const
{
  // Define the sides of the box into which the PMTTorusStack instance would
  // fit.
...
class PMTPolyhedronTorusStack : public G4Polyhedron {
public:
  PMTPolyhedronTorusStack(const G4int n, const G4double z_edge[], const G4double rho_edge[],
...
G4int PMTPolyhedronTorusStack::MakeSegment(G4double rho0, G4double rho1, G4double z0, G4double z1,
...
PMTPolyhedronTorusStack::PMTPolyhedronTorusStack(
    const G4int n, const G4double z_edge[], const G4double rho_edge[], const G4double z_o[],
 ...
 ***********************************************************************/
{
  ...
  if (n <= 0) {
    G4cerr << "Error: bad parameters in PMTPolyhedronTorusStack!" << G4endl;
    G4Exception(__FILE__, "Invalid Parameter", FatalException,
                "PMTPolyhedronTorusStack: bad parameters!");
  }
  ...
  std::vector<G4double> zz(np1 + np2), rr(np1 + np2);

  zz[0] = z_edge[0];
  rr[0] = (inner_n == 0) ? 0.0 : inner_rho_edge[0]; // closes end
  int i, j;
  for (i = 0, j = 1; i < n; i++) {
    j += MakeSegment(rho_edge[i], rho_edge[i + 1], z_edge[i], z_edge[i + 1], z_o[i], a[i], b[i], ns,
                     rr.data() + j, zz.data() + j);
  }
  zz[j] = z_edge[n];
  rr[j] = rho_edge[n];
  ...
  if (j >= np1 - 1) {
    G4Exception(__FILE__, "WTF", FatalException,
                "Logic error in PMTPolyhedronTorusStack, memory corrupted!");
  }
  zz[j] = z_edge[n];
  ...
  if (inner_n == 0) {
    ...
    for (i = inner_n - 1; i >= 0; i--) {
      ...
      j += MakeSegment(inner_rho_edge[i + 1], inner_rho_edge[i], inner_z_edge[i + 1],
                       inner_z_edge[i], inner_z_o[i], inner_a[i], inner_b[i], ns, rr.data() + j,
                       zz.data() + j);
    }
    zz[j] = zz[0];
    rr[j] = rr[0];
  ...
  if (j - np1 > np2) {
    G4Exception(__FILE__, "WTF", FatalException,
                "Logic error 2 in PMTPolyhedronTorusStack, memory corrupted!");
  }
  np2 = j - np1;

  //   R O T A T E    P O L Y L I N E S

  RotateAroundZ(0, 0.0, CLHEP::twopi, np1, np2, zz.data(), rr.data(), -1, 1);
  SetReferences();

  InvertFacets();
}

G4Polyhedron * PMTTorusStack::CreatePolyhedron() const
{
  if (inner == nullptr) {
    return new PMTPolyhedronTorusStack(n, z_edge.data(), rho_edge.data(), z_o.data(), a.data(),
                                        b.data(), 0, nullptr, nullptr, nullptr, nullptr, nullptr);
  }
  else {
    return new PMTPolyhedronTorusStack(n, z_edge.data(), rho_edge.data(), z_o.data(), a.data(),
                                        b.data(), inner->n, inner->z_edge.data(),
                                        inner->rho_edge.data(), inner->z_o.data(), inner->a.data(),
                                        inner->b.data());
  }
}
