...
#include "PMTOpticalModel.hh"
#include "PMTSD.hh"
#include "Randomize.hh"

G4UIdirectory * PMTOpticalModel::fgCmdDir = nullptr;
double PMTOpticalModel::surfaceTolerance = 0.0;

// 28-Jul-2006 WGS: Must define a G4Region for Fast Simulations
// (change from Geant 4.7 to Geant 4.8).
PMTOpticalModel::PMTOpticalModel(G4String modelName, G4Region * envelope_region,
    ...
    _photocathode_MAXrho(photocathode_MAXrho)
{
  ...
  _applyCorrection = (photocathode_MINrho != 0.0 && photocathode_MAXrho != 0.0);
  ...
  if (glass_pt == nullptr) {
    G4Exception(__FILE__, "Bad Properties", FatalException,
                "PMTOpticalModel: glass lacks a properties table!");
  }

  _rindex_glass = glass_pt->GetProperty("RINDEX");
  if (_rindex_glass == nullptr) {
    G4Exception(__FILE__, "Bad Properties", FatalException,
                "PMTOpticalModel: glass does not have RINDEX!");
  }

  // daughter volumes: inner1 is photocathode vacuum, inner2 and central_gap are others
  ...
  if (pc_opsurf == nullptr) {
    G4Exception(__FILE__, "Bad Properties", FatalException,
                "PMTOpticalModel: no photocathode optical surface!?!");
  }

  G4MaterialPropertiesTable * pc_pt = pc_opsurf->GetMaterialPropertiesTable();
  if (pc_pt == nullptr) {
    G4Exception(__FILE__, "Bad Properties", FatalException,
                "PMTOpticalModel: photocathode lacks a properties table!");
  }
...
G4bool PMTOpticalModel::IsApplicable(const G4ParticleDefinition & particleType)
{
...
// -- see also Geant4 docs
G4bool PMTOpticalModel::ModelTrigger(const G4FastTrack & fastTrack)
{
...
void PMTOpticalModel::DoIt(const G4FastTrack & fastTrack, G4FastStep & fastStep)
{
  // Logic summary:
  ...
  // print verbose info
  if (_verbosity > 0) {
    G4cout << "> Enter PMTOpticalModel, ipmt=" << ipmt
           ...
           << "," << _n2 << "," << _k2 << "," << _efficiency << ")" << G4endl;
  }
  _rho = std::hypot(pos.x(), pos.y());
  ...
  for (iloop = 0; iloop < max_iloop; iloop++) {
    ...
    if (_cos_theta1 < 0.0) {
#ifdef G4DEBUG
      G4G4cerr << "PMTOpticalModel::DoIt(): "
               << " The normal points the wrong way!" << G4endl << "  norm: " << norm << G4endl
               << "  dir:  " << dir << G4endl << "  _cos_theta1:  " << _cos_theta1 << G4endl
               << "  pos:  " << pos << G4endl << "  whereAmI:  " << (int)(whereAmI) << G4endl
    ...
    if (A < 0.0 || A > 1.0 || collection_eff < 0.0 || collection_eff > 1.0) {
      G4cout << "PMTOpticalModel::DoIt(): Strange coefficients!" << G4endl;
    ...
    if (EfficiencyCorrection.count(ipmt)) {
      if (EfficiencyCorrection[ipmt] >= 0) { mean_N_pe *= EfficiencyCorrection[ipmt]; }
        ...
        G4cerr << "PMTOpticalModel[" << GetName()
               << "] warning: individual efficiency correction for PMT " << ipmt << " is "
               << EfficiencyCorrection[ipmt] << ", resetting to 1" << G4endl;
    ...
    if (N_pe == 0 && whereAmI == kInVacuum && _prepulseProb > 0.0 && dir.z() < 0.0) {
      ...
      if (std::hypot(posAtDynodeZ.x(), posAtDynodeZ.y()) < _dynodeRadius) {
    ...
    if (N_pe > 0) {
      if (detector != nullptr && detector->isActive())
        ((PMTSD *)detector)->SimpleHit(ipmt, time, energy, pos, dir, pol, N_pe);
      if (_verbosity >= 2) { G4cout << "PMTOpticalModel made " << N_pe << " pe\n"; }
    }
    ...
    if (ranno_absorb < A) {
      weight = 0;
      if (_verbosity >= 2) { G4cout << "PMTOpticalModel absorbed track" << G4endl; }
      break;
    }
    ...
    if (G4UniformRand() < R / (R + T)) { // reflect
      ...
      if (_verbosity >= 2) { G4cout << "PMTOpticalModel reflects track" << G4endl; }
    }
    else { // transmit
      ...
      if (_verbosity >= 2) {
        G4cout << "PMTOpticalModel transmits track, now in "
  ...
  if (weight <= 0) {
    ...
    if (weight < 0) {
      G4cerr << "PMTOpticalModel::DoIt(): Logic error, weight = " << weight << G4endl;
    }
  }
  else {
  ...
  if (iloop >= max_iloop) {
    G4cerr << "PMTOpticalModel::DoIt(): Too many loops, particle trapped! "
              "Killing it."
           << G4endl;
    fastStep.ProposeTrackStatus(fStopAndKill);
  }
  ...
  if (_verbosity > 0) {
    G4cout << "> Exit PMTOpticalModel, ipmt=" << ipmt
           << (whereAmI == kInVacuum ? " vacuum" : " glass") << ", pos=" << pos << ", dir=" << dir
           << ", weight=" << weight << ", pol=" << pol << ", iloop=" << iloop << G4endl;
  }
}
...
{
  const G4complex zi(0., 1.);
  return (1. / zi) * (log(zi * theta + sqrt(1. - theta * theta)));
}
...
void PMTOpticalModel::CalculateCoefficients()
{
  if (_luxlevel <= 0) {
...
#ifdef G4DEBUG
  if (_verbosity >= 10) {
    G4cout << "=> lam, n1, n2: " << _wavelength / nanometer << " " << _n1 << " " << _n2comp
           << G4endl;
...
void PMTOpticalModel::Reflect(G4ThreeVector & dir, G4ThreeVector & pol, G4ThreeVector & norm)
{
  dir -= 2. * (dir * norm) * norm;
  pol -= 2. * (pol * norm) * norm;
}
...
void PMTOpticalModel::Refract(G4ThreeVector & dir, G4ThreeVector & pol, G4ThreeVector & norm)
{
...
void PMTOpticalModel::SetNewValue(G4UIcommand * command, G4String newValues)
{
...
G4String PMTOpticalModel::GetCurrentValue(G4UIcommand * command)
{
  G4String commandName = command->GetCommandName();
  if (commandName == "verbose") { return G4String(std::to_string(_verbosity)); }
  else if (commandName == "luxlevel") {
    return G4String(std::to_string(_luxlevel));
  }
  else {
...
int PMTOpticalModel::GetPMTID(const G4FastTrack & fastTrack)
{
  int iDepth;
